...
import 'package:repsys/ui/planos/components/deletar_plano.dart';
import 'package:repsys/ui/planos/view_models/planos_viewmodel.dart';
import 'package:repsys/utils/constants.dart';

class EditarPlano extends StatefulWidget {
  const EditarPlano({super.key, required this.plano});
  final CorePlanosManutencaoModel plano;

  @override
  State<EditarPlano> createState() => _EditarPlanoState();
}

class _EditarPlanoState extends State<EditarPlano> {
  ...
  final _repository = CorePlanosManutencaoRepository();
  bool _carregandoChecklists = true;

  @override
  void initState() {
    super.initState();
    _tipoPlano = widget.plano.tipoPlano;
  ...
  Future<void> _carregarChecklists() async {
    if (widget.plano.id == null) {
      setState(() => _carregandoChecklists = false);
      return;
    }

    try {
      final checklists = await _repository.buscarChecklistsPorPlano(
        planoManutencaoId: widget.plano.id!,
      );
      ...
      setState(() {
        _checklistGroups.clear();
        _checklistGroups.addAll(grupos);
        _carregandoChecklists = false;
      });
    } catch (e) {
      setState(() => _carregandoChecklists = false);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Erro ao carregar checklists: $e')),
        );
      }
    }
  }
                      ...
                      onPressed: () async {
                        await showDialog(
                          ...
                          builder: (_) => ChangeNotifierProvider(
                            ...
                            child: DeletarPlano(plano: widget.plano),
                          ),
                        );
                      },
                    ),
...
// Classe auxiliar para grupos de checklist (reutilizada do novo_plano.dart)
...
// Classe auxiliar para itens de checklist (reutilizada do novo_plano.dart)
...
// Dialog para adicionar/editar grupo de checklist (reutilizado do novo_plano.dart)
