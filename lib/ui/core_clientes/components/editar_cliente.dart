...
import 'package:repsys/ui/core_clientes/components/deletar_cliente.dart';
...
class _EditarClienteState extends State<EditarCliente> {
  ...
  void initState() {
    ...
    _tipoContrato = widget.cliente.tipoContrato;
    _ativo = widget.cliente.ativo ?? true;
    _nomeController.text = widget.cliente.nome ?? '';
    _emailController.text = widget.cliente.email ?? '';
    _documentoController.text = widget.cliente.documento ?? '';
    _telefoneController.text = widget.cliente.telefone ?? '';
    _valorContratoController.text = widget.cliente.valorContrato != null 
        ? widget.cliente.valorContrato!.toStringAsFixed(2).replaceAll('.', ',')
        : '';
    _diaPagamentoController.text = widget.cliente.diaPagamento?.toString() ?? '';
    _dataAssinaturaController.text = widget.cliente.dataAssinatura != null
        ? '${widget.cliente.dataAssinatura!.day.toString().padLeft(2, '0')}/${widget.cliente.dataAssinatura!.month.toString().padLeft(2, '0')}/${widget.cliente.dataAssinatura!.year}'
        : '';
    _endCepController.text = widget.cliente.endCep ?? '';
    _endRuaController.text = widget.cliente.endRua ?? '';
    _endNumeroController.text = widget.cliente.endNumero ?? '';
    _endBairroController.text = widget.cliente.endBairro ?? '';
    _endCidadeController.text = widget.cliente.endCidade ?? '';
    _endUfController.text = widget.cliente.endUf ?? '';
    _endComplementoController.text = widget.cliente.endComplemento ?? '';
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    IconButton(
                      icon: Icon(Icons.delete, color: AppColors.secondaryText),
                      onPressed: () async {
                        await showDialog(
                          context: context,
                          builder: (_) => ChangeNotifierProvider(
                            create: (_) => CoreClientesViewModel(),
                            child: DeletarCliente(cliente: widget.cliente),
                          ),
                        );
                      },
                    ),
                            ...
                            child: TextButton(
                              onPressed: vm.isSaving
                                  ...
                                  : () async {
                                      ...
                                      final erro = await vm.editar(
                                        itemId: widget.cliente.id,
                                      ...
                                      if (erro == null) {
                                        // Atualiza o AppState com os dados editados
                                        // Parse dos valores
                                        final valorContrato = _valorContratoController.text.isEmpty 
                                            ? null 
                                            : double.tryParse(_valorContratoController.text.replaceAll(RegExp(r'[^\d,]'), '').replaceAll(',', '.'));
                                        
                                        DateTime? dataAssinatura;
                                        if (_dataAssinaturaController.text.isNotEmpty) {
                                          final parts = _dataAssinaturaController.text.split('/');
                                          if (parts.length == 3) {
                                            ...
                                            if (day != null && month != null && year != null) {
                                              try {
                                                dataAssinatura = DateTime(year, month, day);
                                              } catch (e) {
                                                dataAssinatura = null;
                                              }
                                            }
                                        ...
                                        final clienteAtualizado = widget.cliente.copyWith(
                                          ...
                                          diaPagamento: int.tryParse(_diaPagamentoController.text.trim()),
                                        ...
                                        appState.updateCoreClientesModel(coreClientesModel: clienteAtualizado);
